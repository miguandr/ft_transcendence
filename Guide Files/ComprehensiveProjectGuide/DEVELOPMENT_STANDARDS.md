...
## 5. Shared Types (API Contracts)

- All API request/response types live in `/shared/types`
...
4. Frontend updates before merge
...
- Backend handles session validation
...
### ✅ Create New Types in /shared

When you need a new type that both frontend and backend will use:
...
if (data.success && data.data) {
  setUser(data.data.user);  // TypeScript knows structure
}
```
...
@Post('register')
async register(@Body() dto: RegisterRequest): Promise<ApiResponse<AuthResponse>> {
  // Real implementation
